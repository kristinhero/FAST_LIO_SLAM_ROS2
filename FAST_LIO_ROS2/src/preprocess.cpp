...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  if (plsize == 0)
  {
    printf("Error: Empty point cloud received! Please check your LiDAR data and ensure that the point cloud message contains valid points.\n");
    return;
  }
  // Sort points by timestamp to ensure correct time ordering
  ...
  if(t0 == 0)
  {
    printf("Error: No timestamp in point cloud! Please check your LiDAR data and ensure that the timestamp is correctly included.\n");
    return;
  }
