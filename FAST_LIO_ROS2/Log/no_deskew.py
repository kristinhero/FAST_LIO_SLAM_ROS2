...
try:
    ...
    if dk_pos.shape[1] >= 100 and nd_pos.shape[1] >= 100:
        ...
        fig_i, axs_i = plt.subplots(2, 2, figsize=(12, 8))
        fig_i.suptitle('IEKF Information Matrix  |  C0 = deskew,  C1 = no-deskew')

        axs_i[0, 0].plot(dk_tp, dk_pos[:, 61], color='C0', label='deskew')
        axs_i[0, 0].plot(nd_tp, nd_pos[:, 61], color='C1', label='no-deskew')
        axs_i[0, 0].set_title('Effective Points')
        axs_i[0, 0].grid(); axs_i[0, 0].legend(fontsize=7)

        axs_i[0, 1].plot(dk_tp, dk_pos[:, 63], color='C0', label='deskew')
        axs_i[0, 1].plot(nd_tp, nd_pos[:, 63], color='C1', label='no-deskew')
        axs_i[0, 1].set_title('Cost (sum sq res)')
        axs_i[0, 1].grid(); axs_i[0, 1].legend(fontsize=7)

        eig_colors_dk = [f'C{i}'   for i in range(6)]
        eig_colors_nd = [f'C{i+6}' for i in range(6)]
        for i in range(6):
            axs_i[1, 0].plot(dk_tp, dk_eig_vals[:, i], color=eig_colors_dk[i], label=f'dk eig_{i+1}')
            axs_i[1, 0].plot(nd_tp, nd_eig_vals[:, i], color=eig_colors_nd[i], label=f'nd eig_{i+1}')
        axs_i[1, 0].set_title('Info Matrix Eigenvalues (ascending)')
        axs_i[1, 0].set_yscale('log')
        axs_i[1, 0].grid(); axs_i[1, 0].legend(fontsize=6, ncol=2)

        axs_i[1, 1].plot(dk_tp, dk_cond, color='C0', label='deskew')
        axs_i[1, 1].plot(nd_tp, nd_cond, color='C1', label='no-deskew')
        axs_i[1, 1].set_title('Condition Number (eig_min / eig_max)')
        axs_i[1, 1].grid(); axs_i[1, 1].legend(fontsize=7)

        for ax in axs_i.ravel():
            ax.set_xlabel('Time [s]')
        plt.tight_layout()
